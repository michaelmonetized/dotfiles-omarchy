;(function() {
	...
	if (window.location.host.indexOf('tweetdeck.twitter.com') !== 0 && window.location.host.indexOf('tweetdeck.x.com') !== 0) return;
	...
	var conifg = [
        ...
        // Streams 2022 Beta
        {
            placement: 'tweekdeck-stream-2022',
            selector: "article[data-testid='tweet']:not(article[data-buffer='inserted'])",
            $button: $([
                '<div class="tweet-action-item pull-left margin-r--13">',
                '<a class="js-buffer-action tweet-action position-rel" href="#" rel="buffer" title="Buffer">',
                '<i class="icon icon-buffer-beta txt-right"></i>',
                '</a>',
                '</div>'
            ].join('')),
            insert: function(el) {
                ...
                $actions
                    .attr('data-buffer','inserted')
                    .find("div[role='group']:last")
                    .prepend($newActionItem);
            ...
            getData: function(el) {
                ...
                var $text = $streamItem.find("div[data-testid='tweetText']");
                var $screenname = $streamItem.find("div[data-testid='User-Names'] div:nth-child(2) a[role='link']").first();
                var screenname = $screenname.text().trim().replace(/^@/, '');
                var text = getFullTweetText($text, screenname);
                var displayName = $streamItem.find("div[data-testid='User-Names'] div:first a[role='link']").first().text().trim();
                var tweetKey = $streamItem.attr('data-key');
                var $tweetAction;
                
                var retweeted_tweet_id;

                var link = $streamItem.find("div[data-testid='User-Names'] div:last a[role='link']").first().attr('href');
                
                if(link){
                    var linkSplit = link.split('/');
                    retweeted_tweet_id = linkSplit[linkSplit.length-1];
                } else {
                    link = $streamItem.find("time").closest('a').attr('href');
                    var linkSplit = link.split('/');
                    retweeted_tweet_id = linkSplit[linkSplit.length-1];
                }
	...
	var RTConfig = {
		...
		getData: function(el) {
			...
			var tweetId = tweetURL.replace(/https:\/\/twitter.com\/\w+\/status\//, '').replace(/https:\/\/x.com\/\w+\/status\//, '');
	...
	;(function check() {
		chrome.storage.local.get("buffer.op.tweetdeck", function(result) {
			if (result["buffer.op.tweetdeck"] === "tweetdeck") {
				start();
			} else {
